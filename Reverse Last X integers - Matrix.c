/*
The program must accept an integer matrix of size RxC and an integer X as the input. The program must reverse last X integers in each row. Finally, the program must print the modified matrix as the output. 
...
Output Format: The first R lines contain C integers separated by space(s). 
...
The Last three integers in the 2nd row are 11, 11 and 19. Reverse those integers and then print them as the output. 
